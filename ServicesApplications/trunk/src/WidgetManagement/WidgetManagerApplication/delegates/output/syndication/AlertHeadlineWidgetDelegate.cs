    ...
    public class AlertHeadlineWidgetDelegate : AbstractWidgetDelegate, IWidgetPodcastDelegate, IWidgetPreviewDelegate
    {
        ...
        private List<AlertInfo> PopulateHeadlineWidgetDataForPreview(List<int> ids)
        {
            ...
            if (ids.Count > 0)
            {
                ...
                List<int> groupFolders = GetGroupFolders();
                            ...
                            if (folder.PerformContentSearchResponse != null)
                            {
                                    ...
                                    searchResponse.ContentSearchResult.ContentHeadlineResultSet!= null)
                                {
                                        ...
                                        searchResponse.ContentSearchResult.ContentHeadlineResultSet.ContentHeadlineCollection != null)
                                    {
                                        ...
                                        foreach (ContentHeadline contentHeadline in searchResponse.ContentSearchResult.ContentHeadlineResultSet.ContentHeadlineCollection)
                                        {
                                            ...
                                            searchResponse.ContentSearchResult.CodeNavigatorSet.Count > 0)
                                        {
                                            ...
                                            foreach (Navigator navigator in searchResponse.ContentSearchResult.CodeNavigatorSet.NavigatorCollection)
                                            {
                                                ...
                                                switch (navigator.Id.ToLower())
                                                {
                                                    case "in":
                                                        ...
                                                        alertInfo.IndustriesChart.Chart = PopulateDiscoveryChartData(navigator);
                                                        alertInfo.IndustriesChart.GetThisAlertUrl = string.Empty;
                                                        break;
                                                    case "co":
                                                        alertInfo.CompaniesChart = new DiscoveryInfo();
                                                        alertInfo.CompaniesChart.Chart = PopulateDiscoveryChartData(navigator);
                                                        alertInfo.CompaniesChart.GetThisAlertUrl = string.Empty;
                                                        break;
                                                    case "pe":
                                                        alertInfo.ExecutivesChart = new DiscoveryInfo();
                                                        alertInfo.ExecutivesChart.Chart = PopulateDiscoveryChartData(navigator);
                                                        alertInfo.ExecutivesChart.GetThisAlertUrl = string.Empty;
                                                        break;
                                                    case "ns":
                                                        alertInfo.SubjectsChart = new DiscoveryInfo();
                                                        alertInfo.SubjectsChart.Chart = PopulateDiscoveryChartData(navigator);
                                                        alertInfo.SubjectsChart.GetThisAlertUrl = string.Empty;
                                                        break;
                                                    case "re":
                                                        alertInfo.RegionsChart = new DiscoveryInfo();
                                                        alertInfo.RegionsChart.Chart = PopulateDiscoveryChartData(navigator);
                                                        alertInfo.RegionsChart.GetThisAlertUrl = string.Empty;
                                                        break;
                                                    default:
                                                        break;
                                                }
                ...
                if (alertInfos.Count > 0)
                {
                    foreach (int i in ids)
                    {
                        ...
                        else
                        {
                            AlertInfo alertInfo = new AlertInfo();
                            alertInfo.Id = i;
                            alertInfo.Status = GetStatus(new EmgWidgetsUIException(EmgWidgetsUIException.INVALID_FOLDER_ID));
                        ...
                        if (folder.PerformContentSearchResponse != null)
                        {
                                ...
                                searchResponse.ContentSearchResult.ContentHeadlineResultSet != null)
                            {
                                    ...
                                    searchResponse.ContentSearchResult.ContentHeadlineResultSet.ContentHeadlineCollection != null)
                                {
                                    ...
                                    foreach (var contentHeadline in searchResponse.ContentSearchResult.ContentHeadlineResultSet.ContentHeadlineCollection)
                                    {
                                        contentHeadlineStruct.contentHeadline = contentHeadline;
                                        headlineInfos.Add(headlineUtility.ConvertToAlertHeadlineInfo(contentHeadlineStruct));
                                    }
