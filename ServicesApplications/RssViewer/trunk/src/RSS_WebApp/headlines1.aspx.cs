...
namespace FactivaRSS
{
    ...
    /// </summary>
    public partial class headlines1 : System.Web.UI.Page
    {
        ...
        private void getDecryptKey(ref string decryptKey, ref string eidName, ref string eidValue)
        {
            ...
            // The Code below commented out can be used to accomadate the old encryption technology with within this page.
            //  A new config file is required to handle the "folderid" from the old encryption key 
            //				string eid1="";

            //				if (Request["eid1"] != null && Request["feed"] !=null)
            //
            //				{
            //					string[] data = new string[5];
            //					eidName = "eid3";
            //					eidValue = Request["eid1"].ToString();
            //					// UserName | feed | accountID | nameSpace | from
            //					eid1 = GenerateRss.rss.Decrypt(eidValue,ENCRYPTKEY);
            //					data = eid1.Split('|');
            //
            //					inputData.data.Add("userID",data[0]);
            //					inputData.data.Add("feed",data[1]);
            //					inputData.data.Add("accountID",data[2]);
            //					inputData.data.Add("nameSpace",data[3]);
            //					inputData.data.Add("from",data[4].Replace("G","g3"));
            //					eidValue = e.encrypt(inputData.data,eidData.getItem("//" + eidName + "/@key"));
            //
            //				}
            //				else if (Request["fid"] != null && Request["eid1"] !=null)
            //
            //				{
            //					string[] data = new string[6];
            //					eidName = "eid3";
            //					eidValue = Request["eid1"].ToString();
            //					// UserName | fid |folderName| accountID | nameSpace | from
            //					eid1 = GenerateRss.rss.Decrypt(eidValue,ENCRYPTKEY);
            //					data = eid1.Split('|');
            //					inputData.data.Add("userID",data[0]);
            //					inputData.data.Add("fid",data[1]);
            //					inputData.data.Add("folderName",data[2]);
            //					inputData.data.Add("accountID",data[3]);
            //					inputData.data.Add("nameSpace",data[4]);
            //					inputData.data.Add("from",data[5].Replace("G","g1"));
            //					eidValue = e.encrypt(inputData.data,eidData.getItem("//" + eidName + "/@key"));
            //
            //				}
            //				else if (Request["feed"] != null)
            //
            //				{
            //					eidName = "eid3";
            //					foreach(string strrequest in Request.QueryString)
            //					{
            //						inputData.data.Add(strrequest,Request.Params[strrequest]);
            //					}
            //					inputData.data.Add("from","g3");
            //					eidValue = e.encrypt(inputData.data,eidData.getItem("//" + eidName + "/@key"));
            //
            //				}
