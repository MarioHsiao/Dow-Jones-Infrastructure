...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="fn xsl xs xsi">
  ...
  <xsl:template match="/*">
    <xsl:element name="TrackFldrSubscriptionRequest">
      ...
      <xsl:apply-templates select="//emailSync"></xsl:apply-templates>
      <xsl:element name="Subscription">
        ...
        <xsl:apply-templates select="//deliveryTimesList">
        </xsl:apply-templates>
        ...
        <xsl:apply-templates select="//delivContentTypes"></xsl:apply-templates>
  ...
  <xsl:template match="//deduplicationLevel">
    <xsl:element name="DedupLevel">
      ...
      <xsl:attribute name="showdup">
        <xsl:choose>
          <xsl:when test="//showDup='true' or //showDup='1'">1</xsl:when>
          <xsl:otherwise>0</xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
    </xsl:element>
  ...
  <xsl:template match="//emailSync">
    <xsl:element name="EmailSync">
      <xsl:choose>
  ...
  <xsl:template match="//delivContentTypes">
    <xsl:element name="delivContentType">
      <xsl:call-template name="joindelivContentType">
        <xsl:with-param name="listdelivContentType" select="DelivContentType" />
  ...
  <xsl:template name="joindelivContentType">
    <xsl:param name="listdelivContentType" />
    <xsl:param name="separator"/>
    <xsl:for-each select="$listdelivContentType">
      <xsl:value-of select="."/>
  ...
  <!--<xsl:template match="//deliveryTimesList">
    <xsl:for-each select="DeliveryTimes">
      <xsl:element name="DeliveryTimes">
        <xsl:choose>
          <xsl:when test=".='Morning'">
  ...
  <xsl:template match="//deliveryTimesList">
    <xsl:element name="DeliveryTimes">
      <xsl:call-template name="join">
        <xsl:with-param name="list" select="DeliveryTimes" />
        <xsl:with-param name="separator" select="','" />
      </xsl:call-template>
    </xsl:element>
  </xsl:template>
