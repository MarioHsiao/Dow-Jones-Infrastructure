...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="user" extension-element-prefixes="msxsl user" exclude-result-prefixes="user">
	...
	<xsl:include href="../common/commonDistDocElements.xslt"/>
	...
	<xsl:template match="*" mode="ReplyItem">
		<xsl:choose>
			...
			<xsl:when test="(local-name()='GroupInfo')">
				...
				<xsl:if test="count(./DescNL)!=0">
					<xsl:call-template name="descriptionNL"/>
				</xsl:if>-->
			</xsl:when>
			<xsl:when test="(local-name()='SrcInfo')">
				<xsl:if test="count(Groups/Code)!=0">
					<groupCodes>
						<xsl:call-template name="groupCodes"/>
					</groupCodes>
				</xsl:if>
				<xsl:apply-templates select="./*"/>
				<xsl:call-template name="nonPublicationDays"/>
	...
	<xsl:template match="IPPubInfo/*">
		<xsl:choose>
			...
			<xsl:when test="(local-name()='CircDate') and string-length(normalize-space(@v)) &gt; 0">
				<circulationDate>
					<xsl:value-of select="user:ChangeDateFormat(string(normalize-space(@v)))"/>
				</circulationDate>
			</xsl:when>
			...
			<xsl:when test="(local-name()='Publisher')">
				<xsl:if test="string-length(normalize-space(Code/@v)) &gt; 0">
					<publisherCode>
						<xsl:value-of select="normalize-space(Code/@v)"/>
					</publisherCode>
				</xsl:if>
	...
	<xsl:template match="Para">
		<paragraph>
			<xsl:call-template name="commonDistDocElements">
				<xsl:with-param name="processELink">true</xsl:with-param>
			</xsl:call-template>
		</paragraph>
	</xsl:template>
	...
	<!--<xsl:template name="CreateParentGroupCode">
		<xsl:if test="string-length(normalize-space(./Parent[position() = 1]/Code/@v)) &gt; 0">
			<parentGroupCode>
				<xsl:value-of select="normalize-space(./Parent[position() = 1]/Code/@v)"/>
			</parentGroupCode>
		</xsl:if>
	</xsl:template>-->
		...
		<xsl:choose>
			...
			<xsl:when test="(local-name()='Status')">
				...
				<xsl:if test="normalize-space(@v)='discont'">
					<availabilityStatus>Discontinued</availabilityStatus>
				</xsl:if>
			...
			<xsl:when test="(local-name()='DescNL')">
				<xsl:call-template name="descriptionNL"/>
			</xsl:when>
      <xsl:when test="(local-name()='GroupApplicFMT')">
        <groupApplicFMT>
  ...
  <xsl:template match="SrcInfo/*">
    <xsl:choose>
      <xsl:when test="(local-name()='Lang')">
        ...
        <xsl:if test="string-length(normalize-space(@v)) &gt; 0">
          <baseLanguage>
            <xsl:value-of select="user:toLowerCase(string(normalize-space(@v)))"/>
          </baseLanguage>
        </xsl:if>
        <!--</xsl:if>-->
      </xsl:when>
      ...
      <xsl:when test="(local-name()='Online')">
        <xsl:if test="string-length(normalize-space(Date/@v)) &gt; 0">
          <onlineAvailability>
            <xsl:value-of select="user:ChangeDateFormat(string(Date/@v))"/>
          </onlineAvailability>
        </xsl:if>
      </xsl:when>
      ...
      <xsl:when test="(local-name()='ContUPD')">
        <xsl:choose>
          <xsl:when test="string(normalize-space(@v))='N' or string(normalize-space(@v))='n' ">
            <continuouslyUpdated>false</continuouslyUpdated>
          </xsl:when>
          <xsl:when test="string(normalize-space(@v))='Y' or string(normalize-space(@v))='y' ">
            <continuouslyUpdated>true</continuouslyUpdated>
          </xsl:when>
        </xsl:choose>
      </xsl:when>
  ...
  <xsl:template match="Code">
	...
	<xsl:template match="Groups/Code">
		<xsl:choose>
			<xsl:when test="string-length(normalize-space(@v)) &gt; 0">
				<groupCode>
					<xsl:value-of select="normalize-space(@v)"/>
				</groupCode>
			</xsl:when>
