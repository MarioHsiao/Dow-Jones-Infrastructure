...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:user="user"
  ...
  <xsl:template match="//Folder">
    <folderDetails>
      ...
      <userQueryDefinition>
        <xsl:value-of select="UserQueryDefinition"/>
      </userQueryDefinition>
      <userData>
      ...
      <xsl:if test="Delivery/@deliveryMethod != 'online'">
        ...
        <xsl:if test="string-length(normalize-space(Delivery/DeliveryTimes/@time)) &gt; 0">
          <xsl:value-of disable-output-escaping="yes" select="user:DeliveryTimesToList(normalize-space(string(Delivery/DeliveryTimes/@time)))"/>
        </xsl:if>
