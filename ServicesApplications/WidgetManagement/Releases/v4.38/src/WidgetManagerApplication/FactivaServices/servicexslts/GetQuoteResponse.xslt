...
<!--					<Status>
			...
			<xsl:otherwise>
				<quote>
					<xsl:attribute name="timeliness"><xsl:value-of select="@data_quality"/></xsl:attribute>
					<xsl:apply-templates select="Sample/InstrumentName"/>
					<instrumentName/> <!-- Blank instrumentName, will be filled in C# code -->
	...
	<xsl:template match="Sample/InstrumentName">
		<xsl:if test="string-length(normalize-space(.)) &gt; 0">
			<companyName><xsl:value-of select="normalize-space(.)"/></companyName>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="req_token">
		<xsl:if test="string-length(normalize-space(@req_token)) &gt; 0">
			<requestedSymbol><xsl:value-of select="normalize-space(@req_token)"/></requestedSymbol>
			<djTicker><xsl:value-of select="normalize-space(@req_token)"/></djTicker>
		</xsl:if>
	</xsl:template>
	<xsl:template match="Sample/Exchange">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<exchange><xsl:value-of select="normalize-space(@value)"/></exchange>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	<xsl:template match="Sample/Currency">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<currency><xsl:value-of select="normalize-space(@value)"/></currency>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	<xsl:template match="Sample/SpotRateInverted">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<spotRateInverted><xsl:value-of select="normalize-space(@value)"/></spotRateInverted>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/InstrumentType">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<debtType><xsl:value-of select="normalize-space(@value)"/></debtType>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='DividendDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<dividendDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></dividendDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='exDividendDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<exDividendDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></exDividendDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/FreqInterestPayment">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<interestPaymentFrequency><xsl:value-of select="normalize-space(@value)"/></interestPaymentFrequency>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='IssueDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<issueDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></issueDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/DateTime[@fid='LastTradeTime']">
		<xsl:if test="string-length(normalize-space(@date))+string-length(normalize-space(@time))  &gt; 0">
			<xsl:if test="string(number(@date))!='NaN' and  string(number(@time))!='NaN'">
				<lastTradeDateTime><xsl:value-of select="(user:ChangeDateFormat(string(@date)))"/>T<xsl:value-of select="(user:ChangeTimeFormat(string(@time)))"/></lastTradeDateTime>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='LifeHighDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<lifetimeHighDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></lifetimeHighDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='LifeLowDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<lifetimeLowDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></lifetimeLowDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='BondMaturesDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<maturityDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></maturityDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='NextInterestDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<nextInterestDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></nextInterestDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='YearHighDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<last52WeekHighDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></last52WeekHighDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='YearLowDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<last52WeekLowDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></last52WeekLowDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Date[@fid='PreviousNetAssetValueDate']">
		<xsl:if test="string(number(@value))!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<previousNetAssetValueDate><xsl:value-of select="(user:ChangeDateFormat(string(@value)))"/></previousNetAssetValueDate>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Rating">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<rating>	<xsl:value-of select="normalize-space(@value)"/></rating>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/RatingID">
		<xsl:if test="@value!='-'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<ratingID><xsl:value-of select="normalize-space(@value)"/></ratingID>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/DateTime[@fid='UpdateTime']">
		<xsl:if test="string-length(normalize-space(@date))+string-length(normalize-space(@time))  &gt; 0">
			<updateTime><xsl:value-of select="(user:ChangeDateFormat(string(@date)))"/>T<xsl:value-of select="(user:ChangeTimeFormat(string(@time)))"/></updateTime>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Sample/Num[@fid='Yield']">
		<!--<xsl:if test="@value!='-'">--><xsl:if test="number('-')!='NaN'">
			<xsl:if test="string-length(normalize-space(@value)) &gt; 0">
				<yield>
					<xsl:value-of select="user:FormatNumberToPrecision(string(@exp),normalize-space(@value))"/>
					<!--<xsl:if test="@exp!=''">
