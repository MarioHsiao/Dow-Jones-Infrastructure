...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"  xmlns:user="user" extension-element-prefixes="msxsl user">
	...
	<xsl:include href="commonDistDocElements.xslt"/>
			...
			<contentParts>
				...
				<xsl:attribute name="primaryReference">
					<xsl:choose>
						...
						<xsl:when test="@adoctype='multimedia'">
							<xsl:choose>
								<xsl:when test="count(Item[@type='audio']) &gt; 0">
									<xsl:value-of select="Item[@type='audio']/@ref"/>
								</xsl:when>
								<xsl:when test="count(Item[@type='video']) &gt; 0">
									<xsl:value-of select="Item[@type='video']/@ref"/>
								</xsl:when>
							</xsl:choose>
						</xsl:when>
				...
				<part>
					...
					<xsl:if test="string-length(@mimetype)!=0 or @type='ivtxco' or @type='ivtxin'">
						...
						<xsl:choose>
							...
							<xsl:otherwise><xsl:attribute name="mimeType"><xsl:value-of select="@mimetype"/></xsl:attribute></xsl:otherwise>
						</xsl:choose>
					</xsl:if>
					...
					<xsl:attribute name="reference">
						<xsl:choose>
							<xsl:when test="parent::AdocTOC/@adoctype='file' and @type='html'"><xsl:value-of select="user:ReplaceStr(string(@ref), 'probj:archive', 'probj_text_html:archive')"/>/<xsl:value-of select="ancestor::Result/@accessionno"/></xsl:when>
							<xsl:otherwise>
								<xsl:choose>
