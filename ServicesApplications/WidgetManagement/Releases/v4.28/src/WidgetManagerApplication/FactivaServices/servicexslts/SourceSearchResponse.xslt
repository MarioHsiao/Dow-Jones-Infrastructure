...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"  xmlns:user="user" extension-element-prefixes="msxsl user" exclude-result-prefixes="user">
	...
	<xsl:template name="ResultSet">
		<xsl:choose>
			<xsl:when test="string-length(normalize-space(//ResultSet/@total)) &gt; 0"><queryHitCount><xsl:value-of select="number(normalize-space(//ResultSet/@total))"/></queryHitCount></xsl:when>
			<xsl:otherwise><queryHitCount>0</queryHitCount>
</xsl:otherwise>
		</xsl:choose>
		<xsl:choose>
			<xsl:when test="string-length(normalize-space(//ResultSet/@first)) &gt; 0"><indexOfFirstHeadline><xsl:value-of select="number(normalize-space(//ResultSet/@first))"/></indexOfFirstHeadline></xsl:when>
			<xsl:otherwise><indexOfFirstHeadline>0</indexOfFirstHeadline></xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="Result">
		...
		<xsl:attribute name="lang">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="user:toLowerCase(string(.//DocData/InterfaceLang/@v))"/>	
				</xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
			<xsl:apply-templates select="SourceDoc/ReplyItem"/>
		<xsl:if test="string-length(normalize-space(LatestIssueDate/@v)) &gt; 0">
			<mostRecentIssue><xsl:value-of select="user:ChangeDateFormat(string(normalize-space(LatestIssueDate/@v)))"/></mostRecentIssue>
		</xsl:if>
		</source>
