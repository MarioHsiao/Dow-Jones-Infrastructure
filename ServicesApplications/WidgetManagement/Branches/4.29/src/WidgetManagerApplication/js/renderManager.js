        ...
        function PrivateConstructor() {
            ...
            this.xTabPanelGroup = function (id, w, h, th, clsTP, clsTG, clsTD, clsTS, index) // object prototype
            {
                ...
                this.onResize = function (newW, newH) {
                    ...
                    /*
                    // iGoogle needs different sizing than rest of endpoints in firefox. 
                    if (!xIE4Up && typeof(_IG_AdjustIFrameHeight) != "undefined"){
                    for(var u=0,sum=0;u<tabSizeArray.length;sum+=tabSizeArray[u++]+2);
                    }
                    else
                    {
                    for(var u=0,sum=0;u<tabSizeArray.length;sum+=tabSizeArray[u++]);
                    ...
                    if (sum > w) {
                        ...
                        */
                        ...
                        if (leftTab != null) {
                            ...
                            this.xWidth(leftTab, gutterWidth);
                        }
                        ...
                        if (rightTab != null) {
                            ...
                            this.xWidth(rightTab, gutterWidth);
                        }
                ...
                if (!FactivaWidgetRenderManager.getInstance().xTabPanelGroup.instances[id].addedListener) {
                    ...
                    function () {
                        ...
                        for (var j = 0; j < tabs.length; j++) {
                            ...
                            if (!xIE4Up)
                            */
                            tabs[j].style.width = "auto";
                        }

                        var cDiv = this.xGetElementById("fctv_tabgroup_outer" + index);
                        var leftBtn = this.xGetElementById("fctv_tabLeft" + index);
                        var rightBtn = this.xGetElementById("fctv_tabRight" + index);
                        var cDivRight = this.xLeft(cDiv) + this.xWidth(cDiv);
                        var tDivRight = this.xLeft(tabGrp[0]) + this.xWidth(tabGrp[0]);

                        // make this condition have a radius to account for browser differences in positioning
                        if (tDivRight < cDivRight) {
                            this.xLeft(tabGrp[0], tabGrp[0].offsetLeft + (cDivRight - tDivRight));
                            for (var k = 0; k < tabs.length; k++) {
                                // reset the left carousel index: find which tab is not 
                                // showing completely starting from the left set to that index

                                if ((tabGrp[0].offsetLeft + tabs[k].offsetLeft) < this.xWidth(leftBtn)) {
                                    var newK = (k == 0) ? 0 : k - 1;

                                    tabGrp[0].leftCarIndex = newK;
                                    tabGrp[0].edgeDistance = this.xWidth(tabs[newK]) - (tabGrp[0].offsetLeft + tabs[k].offsetLeft);
                        ...
                        if (this.xWidth(cDiv) > this.xWidth(tabGrp[0])) {
            ...
            this.appendTabStyles = function (tEle, mColor, tabmColor, showTitle, aColor, bgColor, leftCarouselIcon, rightCarouselIcon) {
                ...
                var tabStyles = "#" + this.xGetElementById(tEle).id + " .tabPanelGroup{overflow:hidden;zoom:1" +
                                    ...
                                    ";} " +
                                "#" + this.xGetElementById(tEle).id + " .tabPanel{display:block;float:left;overflow:hidden;position:relative;width:100%;}" +
                                "#" + this.xGetElementById(tEle).id + " .tabGroupOuterContainer{display:inline-block;float:left;margin-bottom:10px;overflow:hidden;position:relative;width:100%;}" +
...
var DiscoveryChart = (function () {
    ...
    var template = ['<ul class="discovery-items">',
                            '{{ var data = it, mw = 1;',
                                ...
                                '<li class="{{=c}}" data-di="{{=i}}" title="{{=x.name }}">', //background change for source family 
                                    '<span class="dj_not" title="{{="notTitleTkn"}}"><span></span></span>',
                                    '<span class="discovery-chart">',
                                        '<img  class="plot" src="../img/plotdot.png" style="width: {{=w}}px;" />', '<br />',
                                        '<span class="ellipsis">{{=x.name}}</span>',
                                        '<span class="chart-value">{{=x.value}}</span>',
                                    '</span>',
                                '</li>',
                            '{{ } }}',
                        '</ul>',
    ...
    var templateIE = ['<ul class="discovery-items">',
                            '{{ var data = it, mw = 1;',
                                'for (var x=0, w=0, c=0, i = 0, len = data.length;i < len; i++) { ',
                                'x = data[i];w=((i != 0)?((x.value/data[0].value)*170):170); if(w < mw) w=mw; c = x.GT=="sf"? "cItem source-family":"cItem";}}',
                                '<li class="{{=c}}" data-di="{{=i}}" title="{{=x.name }}">', //background change for source family 
                                    '<span class="dj_not" title="{{="notTitleTkn"}}"><span></span></span>',
                                    '<span class="discovery-chart">',
                                        '<img  class="plot" src="../img/plotdot.png" style="float:left;width: {{=w}}px;" />', '<br />',
                                        '<span class="ellipsis" style="float:left;margin-bottom:2px;">{{=x.name}}</span>',
                                        '<span class="chart-value">{{=x.value}}</span>',
                                    '</span>',
                                '</li>',
                            '{{ } }}',
