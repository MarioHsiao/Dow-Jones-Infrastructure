...
{
    public interface IStructuredQueryFactory
    {
        IStructuredQuery Create(CodedStructuredSearchRequest request);
    }

    public class StructuredQueryFactory : IStructuredQueryFactory
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(StructuredQueryFactory));

        public IStructuredQuery Create(CodedStructuredSearchRequest request)
        {
            ...
            // not null only for personalization
            if (request.SearchStringCollection != null && request.SearchStringCollection.Count > 0)
            {
                if (query.Query.SearchStringCollection == null)
                    query.Query.SearchStringCollection = new SearchStringCollection();

                query.Query.SearchStringCollection.AddRange(request.SearchStringCollection);
            }
