...
namespace DowJones.Assemblers.Track
{
    public class TrackDeletedFoldersCacheManager : AbstractAggregationManager
    {
        ...
        public void TrackDeletedFoldersScheduler(string key, object value, CacheItemRemovedReason reason)
        {
            var t = new Thread(() =>
            {
                ...
                try
                {
                    ServiceResponse serviceResponse = TrackService.DeletedFolders(ControlDataManager.Convert(ControlData), new DeletedFoldersRequest());
