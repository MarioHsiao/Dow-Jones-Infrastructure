DJ.$dj.define('AbstractCanvas', ['$', '$dj', '_', 'JSON', 'error-manager', 'jquery-ui-interactions'], function (jQuery, $dj, _, JSON) {
    var $ = jQuery;
    DJ.UI.AbstractCanvas = DJ.UI.CompositeComponent.extend({
        layout: null, _moduleSelector: ".dj_module", init: function (element, meta) {
            ...
            $.ajax({
                url: this.options.deleteModuleUrl + '?' + $.param(request), type: 'DELETE', success: $dj.delegate(this, function (data) {
            ...
            var moduleIds = _.map(modules, function (module) {
                return module.get_moduleId();
            });
            return moduleIds;
        }, getModules: function () {
            ...
            if (!this.canAddModule(moduleId)) {
                return;
            }
            addModule = addModule || this._delegates.addModule;
            var request = { 'pageId': this.options.canvasId, 'id': moduleId };
            ...
            $.ajax({
                url: this.options.loadModuleUrl + '?' + $.param(request), cache: false, complete: $dj.delegate(this, function (xhr) {
            ...
            if (!module)
                return;
            module.showLoadingArea();
            this.loadModule(moduleId, null, function (err) {
            ...
            this._delegates = { addModule: $dj.delegate(this, this.addModule), fireModuleAdded: $dj.delegate(this, this._fireModuleAdded), fireModuleRemoved: $dj.delegate(this, this._fireModuleRemoved) };
    ...
    DJ.UI.Canvas.Layout = DJ.Component.extend({
        _moduleSelector: '.dj_module', init: function (canvas, options) {
    ...
    $.extend(DJ.UI.Canvas.Layout, {
        _layouts: {}, initialize: function (canvas, options) {
            var name = (options || {}).name || 'zone';
            var initLayout = $dj.delegate(this, function () {
                this._initialize(canvas, options);
            });
            if (name == 'zone')
                initLayout();
            else
    ...
    DJ.UI.Canvas.ZoneLayout = DJ.UI.Canvas.Layout.extend({
        defaults: { dataServiceUrl: null, mode: 'column', zones: null, sortableSettings: { axis: false, containment: false, delay: 100, distance: 30, draggingClass: "dj_module-dragging", forcePlaceholderSize: true, handle: ".dj_module-handle", items: "> .dj_module-movable", opacity: 0.8, placeholder: "dj_module-placeholder", revert: 300, scroll: true, tolerance: "pointer" } }, _zoneSelector: '.dj_group', init: function (canvas, options) {
            ...
            $.ajax({ url: this.options.dataServiceUrl, type: 'PUT', data: JSON.stringify(request) }).done(function () {
                resizeZones();
            ...
            if (zones.length !== zoneCount) {
                ...
                for (var i = 0; i < zoneCount; i++) {
                    var zone = $("<div />").addClass(zoneClass).attr('id', 'zone-' + i).appendTo(this.element);
                    zones.push(zone);
                }
            }
            return $(zones);
            ...
            this._delegates = { getZones: $dj.delegate(this, this._getZones), move: $dj.delegate(this, this._move), save: $dj.delegate(this, this.save), resizeZones: $dj.delegate(this, this._resizeZones) };
        }, _initializeModules: function () {
            ...
            if (!zones) {
                ...
                _.each(_.sortBy(modules, function (mod) {
                    return mod.options.position;
                }), function (mod) {
