DJ.$dj.define('autocomplete-plugin', ['$','$dj','_','JSON'], function(jQuery, $dj, _, JSON) { var $ = jQuery; 
 /*
...
;(function($) {
    $.fn.extend({
        _djAutocomplete: function(urlOrData, options) {
            ...
            options = $.extend({}, $.DJAutocompleter.defaults, {
                ...
                delay: isUrl ? $.DJAutocompleter.defaults.delay : 10,
                max: options && !options.scroll ? options.max : 150
            }, options);
    ...
    $.DJAutocompleter = function(input, options) {
        ...
        function selectCurrent() {
            ...
            if (selected.isViewAll) {
                $input.trigger("viewall", {
                    //url: options.url,
                    text: $input.val()
                });
                $input.val("");
            ...
            if (options.multiple) {
                ...
                if (words.length > 1) {
                    ...
                    //$.DJAutocompleter.Selection(input, progress + seperator, progress + seperator);
    ...
    $.DJAutocompleter.defaults = {
        inputClass: "ac_input",
        resultsClass: "dj_emg_autosuggest_results",
        loadingClass: "dj_emg_autosuggest_loading",
    ...
    $.DJAutocompleter.Cache = function(options) {
    ...
    $.DJAutocompleter.Select = function(options, input, select, config) {
        var CLASSES = {
            ACTIVE: options.resultsOverClass
        };
        ...
        function init() {
            ...
            list = $("<table/>").appendTo(element).mouseover(function(event) {
                if (target(event).nodeName && target(event).nodeName.toUpperCase() == 'TR') {
                    ...
                    if (!$(target(event)).hasClass("ac_cat_head") && !$(target(event)).hasClass("ac_cat_foot"))
                        $(target(event)).addClass(CLASSES.ACTIVE);
                }
            }).click(function(event) {
                //$(target(event)).addClass(CLASSES.ACTIVE);
                if (event.target.tagName == "TD" || event.target.tagName == "TR" || event.target.tagName == "STRONG" || event.target.tagName == "DIV" || event.target.tagName == "SPAN") {
                    select();
        ...
        function fillList() {
            ...
            if (data) {
                ...
                for (i = 0; i < max; i++) {
                    ...
                    if (data[i].data.isCategory === true) {
                        ...
                        if ($("tbody", element).find('tr.ac_cat_head_' + data[i].data.controlType.toLowerCase()).get(0) === undefined) {
                            var catHeadHtml = "<tr class=\"ac_cat_head ac_cat_head_" + data[i].data.controlType.toLowerCase() + "\"><td>" + data[i].data.groupHeading + "</td></tr>";
                            $('tbody>tr:last', element).before(catHeadHtml);
                        }
                        //Append the footer
                ...
                //Add class to last source family row if it is not the final.
                var sourceFamilyRows = $('div.acSource_sourcefamily', element).closest('tr');
                if ($(sourceFamilyRows).length != options.maxResults) {
                    $(sourceFamilyRows).last().addClass('ac_sourceFamily_last');
                }
            }
        ...
        return {
            ...
            show: function() {
                ...
                element.css({
                    width: typeof options.width == "string" || options.width > 0 ? options.width : ($(input).outerWidth(false) - 2),
