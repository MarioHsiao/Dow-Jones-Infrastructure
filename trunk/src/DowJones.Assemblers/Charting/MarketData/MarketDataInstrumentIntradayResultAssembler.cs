...
namespace DowJones.Assemblers.Charting.MarketData
{
    public class MarketDataInstrumentIntradayResultAssembler : AbstractMarketDataIntrumentResultAssembler, IAssembler<MarketDataInstrumentIntradayResult, ChartDataResponse>
    {
        ...
        {
            return (from kvp in source.Data let session = kvp.Value.Sessions[0] select GetMarketDataInstrumentIntradayResult(kvp.Key, kvp.Value.Name, System.Convert.ToBoolean(kvp.Value.IsIndex), session)).FirstOrDefault();
        }

        public MarketDataInstrumentIntradayResult Convert(ChartDataResponse source, int incrementInMinutes = 15)
        {
            return (from kvp in source.Data let session = kvp.Value.Sessions[0] select GetMarketDataInstrumentIntradayResult(kvp.Key, kvp.Value.Name, System.Convert.ToBoolean(kvp.Value.IsIndex), session, incrementInMinutes)).FirstOrDefault();
        }
    }
}
