...
DJ.UI.DiscoveryFilters = DJ.UI.Component.extend({
    ...
    selectors: {
        ...
        expandCollapseBtn: 'div.dj_df-header .side-section-title',
        headerContainer: 'div.dj_df-header',
    ...
    */
    ...
    _initializeExpandCollapse: function () {
        var self = this;
        $(this.selectors.expandCollapseBtn).on('click', function () {
    ...
    _initializeDiscoveryFilters: function (discoveryEntityObj) {
        ...
        _.each(discoveryDataArr, function (dd) {
            ...
            if (!dd.isExpanded) {
                //var container = $this.$element.find('.dj_discoveryFilters-listItem-' + dd.position);
                var header = $($this.selectors.headerContainer, container);
                header.addClass("open");
                var dataContainer = $($this.selectors.dataContainer, container);
                dataContainer.hide();
            }
            discoveryLi = $this.$element.find('.dj_discoveryFilters-listItem-' + dd.position);
            //attach export click
            $($this.selectors.exportLink, container).on('click', function () {
                //alert("here");
                //return $this._delegates.OnExportClicked;
                //var self = this;
                $this.publish($this.events.exportClick, { "data": "test export" });
            });
    ...
    */
        ...
        return new Highcharts.Chart($.extend(true, {}, this.discoveryFitlersConfig, {
            ...
            series: [{
                ...
                dataLabels: {
                    ...
                    formatter: function () {
                    if (this.point.name < 10000) {
                    //return this.point.name;
                    return new NumberFormatter().Format(this.point.name, NumberFormatType.Whole);
                    }
                    if (this.point.name <= 99999) {
                    //return String.Format("{0:##.0K}", Math.Round((double)this.point.name / 1000, 1));
                    return String.Format("{0:##.0K}", Math.Round(this.point.name / 1000, 1));
                    }
                    if (this.point.name <= 999999) {
                    //return String.Format("{0:###K}", Math.Round((double)this.point.name / 1000));
                    return String.Format("{0:###K}", Math.Round(this.point.name / 1000));
                    }
                    //if (this.point.name <= 9999999) {
                    //    return String.Format("{0:#.0M}", Math.Round((double)this.point.name / 1000000,1));
                    //}
                    //return String.Format("{0}M", new NumberFormatter().Format(Math.Round((double)this.point.name / 1000000, 1), NumberFormatType.Precision));
                    //return String.Format("{0}M", new NumberFormatter().Format(Math.Round(this.point.name / 1000000, 1), NumberFormatType.Precision));
                    return String.Format("{0}M", Math.Round(this.point.name / 1000000, 1).toFixed(2));
                    }
                    */
                    /*style: {
                    fontWeight: "normal",
    ...
    _renderDiscoveryDateFilters: function (discoveryData, idx) {
        ...
        if (self.options.isDatePointWidthSpecified) {
            this.discoveryFitlersDateChartConfig.plotOptions.series.pointWidth = self.options.datePointWidth; //This is to ensure that the columns are not too wide when there are less columns
        }
