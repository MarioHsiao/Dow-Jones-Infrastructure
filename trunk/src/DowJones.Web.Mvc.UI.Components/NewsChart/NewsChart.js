...
DJ.UI.NewsChart = DJ.UI.Component.extend({
    ...
    events: {
        zoom: "chartZoom.dj.NewsChart",
        zoomReset: "chartZoomReset.dj.NewsChart",
        pointClick: "pointClick.dj.NewsChart",
        sourceClick: "sourceClick.dj.NewsChart"
    },
    ...
    trasformNewsDataResult: function (newsSeries, stockSeries) {
        ...
        if (newsSeries && newsSeries.dataPoints) {
            $.each(newsSeries.dataPoints, function (i, objvalue) {
                if (_.isDate(objvalue.date)) {
                    newsDataSeries.push({x:objvalue.date.getTime(), y:objvalue.dataPoint.value, dateDisplay: objvalue.dateDisplay});                        
                }
                else{ 
                    newsDataSeries.push({x:JSON.parseDate(objvalue.date).getTime(), y:objvalue.dataPoint.value, dateDisplay: objvalue.dateDisplay});
                } 
            });
        }
    ...
    showNewsChart: function (chartOptions) {
        try {
    ...
    renderChart: function () {
        ...
        if (self.data) {
            ...
            this.showNewsChart(chartOptions);
        }
    },
