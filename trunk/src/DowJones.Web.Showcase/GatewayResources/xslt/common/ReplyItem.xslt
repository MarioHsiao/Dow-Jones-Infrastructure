...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"  xmlns:user="user" extension-element-prefixes="msxsl user">
  ...
  <msxsl:script language="JScript" implements-prefix="user">
    ...
    function ChangeTimeFormatWithZ(TimeVal)
		{
			//var RetVal = new String("");
			//RetVal = TimeVal.substr(0,2)+":"+TimeVal.substr(2,2)+":"+TimeVal.substr(4,2)+"Z";
			//return RetVal;
			return ChangeTimeFormat(TimeVal)+"Z";
		}
    ...
    <xsl:choose>
      ...
      <xsl:when test="(local-name()='Date')">
        <xsl:choose>
          <xsl:when test="string-length(normalize-space(@value)) &gt; 0">
            <publicationDate>
              <xsl:value-of select="normalize-space(user:ChangeDateFormat(string(@value)))" />
            </publicationDate>
          </xsl:when>
          <xsl:otherwise>
            <publicationDate/>
      ...
      <xsl:when test="(local-name()='Time')">
        <xsl:if test="string-length(normalize-space(@value)) &gt; 0">
          <publicationTime>
            <xsl:value-of select="user:ChangeTimeFormat(normalize-space(@value))" />
          </publicationTime>
        </xsl:if>
      </xsl:when>
          ...
          <xsl:if test="string-length(normalize-space(.)) &gt; 0">
            <xsl:if test="$type=''">
              <xsl:element name="sectionName">
                <xsl:call-template name="commonDistDocElements"/>
                <!--<xsl:value-of select="normalize-space(SectionName)"/>-->
              </xsl:element>
            </xsl:if>
          </xsl:if>
        </xsl:when>
