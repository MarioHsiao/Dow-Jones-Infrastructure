...
<xsl:template match="//member[contains(@name,'T:')]">
  ...
  <H3><xsl:value-of select="$MemberName"/></H3>
  <xsl:apply-templates/>
  ...
  <xsl:if test="//member[contains(@name,concat('F:',$FullMemberName))]">
   <H4>Fields</H4>
      ...
      <xsl:for-each select="//member[contains(@name,concat('F:',$FullMemberName))]">
        <H5><xsl:value-of select="substring-after(@name, concat('F:',$FullMemberName,'.'))"/></H5>
        <xsl:apply-templates/>
      </xsl:for-each>
  </xsl:if>
  ...
  <H4>Properties</H4>
      ...
      <xsl:for-each select="//member[contains(@name,concat('P:',$FullMemberName))]">
        <H5><xsl:value-of select="substring-after(@name, concat('P:',$FullMemberName,'.'))"/></H5>
        <xsl:apply-templates/>
      </xsl:for-each>
  </xsl:if>
