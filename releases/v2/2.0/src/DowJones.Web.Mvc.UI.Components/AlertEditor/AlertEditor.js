    ...
    DJ.UI.AlertEditor = DJ.UI.Component.extend({
        ...
        */
        ...
        _initializeControls: function () {
            ...
            this.$alertDeliveryDD = this.$alertForm.children(this.selectors.delivery).children("select");
            this.$toggleSwitch = this.$alertForm.children(this.selectors.duplicates).children(this.selectors.toggleSwitch);
        ...
        _initializeEventHandlers: function () {
            ...
            // Bind Alert Delivery Times
            this._bindAlertDeliveryTimes(this.options.deliveryTimes);
            this.$alertDeliveryDD.selectbox().change(this._delegates.OnDeliveryTimeChange);

            // Toggle Duplicates Switch
            this.$alertForm.delegate(this.selectors.toggleSwitch, 'click', function () {
                var $this = $(this);
                me._toggleDuplicates($this.children("span.switch").hasClass("on"));
            });
        ...
        _bindAlertDeliveryTimes: function (dlvr) {
            this.$alertDeliveryDD.empty();
            if (dlvr) {
                for (var i = 0; i < dlvr.length; i++) {
                    this.$alertDeliveryDD.append('<option value="' + dlvr[i].code + '">' + dlvr[i].desc + '</option>');
                }
            }
        },

        _toggleDuplicates: function (ON) {
            var $switch = this.$toggleSwitch.children("span.switch");
            ON ? $switch.removeClass("on") : $switch.addClass("on");
            $switch.html(this.$toggleSwitch.find('> .text-behind > span:' + ($switch.hasClass("on") ? 'last' : 'first')).html());
        },

        _onDeliveryTimeChange: function () {
            if (this.$alertDeliveryDD[0].selectedIndex == 0) {
        ...
        getAlertRequestObject: function () {
            ...
            if (errorMsgs.length == 0) {
                request.properties = {
                    ...
                    deliveryTime: this.$alertDeliveryDD.val(),
                    removeDuplicate: (this.$alertForm.find(this.selectors.toggleSwitch).children("span.switch").hasClass("on")
        ...
        setData: function (data) {
            ...
            this.$alertDeliveryDD.val(alertProperties.deliveryTime).change();

            this._toggleDuplicates(alertProperties.removeDuplicate != this.removeDuplicate.Off);
        }
    });
