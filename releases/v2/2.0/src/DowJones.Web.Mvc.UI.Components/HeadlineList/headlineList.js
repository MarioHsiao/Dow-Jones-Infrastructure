...
(function ($) {
    DJ.UI.HeadlineList = DJ.UI.Component.extend({
        ...
        _initializeEventHandlers: function () {
            ...
            this.$headlineBox.delegate(self.selectors.author, 'click', function (e) {
                ...
                if (entityCode) {
                    ...
                    $dj.debug('Author Code not found. Need not publish event.')
                }
                return false;
            });
        ...
        toggleDups: function (headlineChk, selectDups) {
            var $chk = (headlineChk.jquery) ? headlineChk : $(headlineChk),
                checked = (selectDups === undefined) ? $chk.is(':checked') : selectDups,
                dupWrapper = $chk.closest('li').children(this.selectors.dupWrapper),
                dupWrapperHeader = dupWrapper.children('p');

            switch (selectDups) {
                case 'none': dupWrapper.find(this.selectors.dupHeadlineCheckbox).prop('checked', false);
                    return true;

                case 'visible': checked = this._dupsTrayOpen(dupWrapperHeader);
                ...
                default: if (this._dupsTrayOpen(dupWrapperHeader)) {
            ...
            //            // check if dups tray is open. if yes, then toggle them
            //            // if selectDups options is specified, toggle dups anyway
            //            if (this._dupsTrayOpen(dupWrapperHeader) || (selectDups !== undefined)) {
            //                dupWrapper.find(this.selectors.dupHeadlineCheckbox).attr('checked', checked);
            //                return true;
            //            }
        },
        ...
        getSelectedHeadlinesData: function () {
            ...
            for (var i = 0, len = dupHeadlines.length; i < len; i++) {
                data.push(this._getHeadlineData(headlines[i]));
            }
