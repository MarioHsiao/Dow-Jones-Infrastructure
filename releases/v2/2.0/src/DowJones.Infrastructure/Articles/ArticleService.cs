...
namespace DowJones.Articles
{
    public class ArticleService : AbstractAggregationManager, IArticleService
    {
        ...
        public ArticleService(IControlData controlData) : base(controlData)
        {
        }              
        ...
        public ArticleResponseSet GetArticles(MixedContentArticleRequest request)
        {
            ...
            if (searchAns.Count > 0)
            {
                ...
                var searchRequest = GetPerformContentSearchRequest<PerformContentSearchRequest>(searchAns);
                var sr = Process<PerformContentSearchResponse>(searchRequest);
                if( sr != null &&
        ...
        private T GetPerformContentSearchRequest<T>(IList<string> ans) where T : IPerformContentSearchRequest, new()
        {
            var request = new T {FirstResult = 0, MaxResults=100};

            var srch = new StructuredSearch
                           {
                               Formatting =
                                   {
                                   ...
                                   }, Query = {Dates = new Dates {All = true}}
                           };


            srch.Query.SearchStringCollection.Add(
                new SearchString
                    {
                        Mode = SearchMode.Any,
                        Scope = "an",
                        Value = String.Join(" ", ans),
                        Type = SearchType.Controlled,
                    }
                );
            request.StructuredSearch = srch;

            //TODO: Replace with PST naresh!


            request.StructuredSearch.Query.SearchCollectionCollection.AddRange(new[]
                                                                                   {
        ...
        private ArticleResponseSet GetArticlesWithLimit(GetArticleRequest request)
        {
            ...
            {
                limitRequest = new GetArticleWithLimitRequest { continuationContext = continuationContext };
                var eachResponse = GetArticlesWithLimit(limitRequest);
