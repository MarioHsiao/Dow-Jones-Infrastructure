...
namespace DowJones.Assemblers.Headlines
{
    internal class PerformContentSearchResponseConverter : AbstractHeadlineListDataResultSetConverter, IExtendedListDataResultConverter
    {
        ...
        protected internal void Convert(HeadlineInfo headlineInfo, ContentHeadline contentHeadline, int index, bool isDuplicate)
        {
            ...
            headlineInfo.thumbnailImage = this.GetThumbnailImage(contentHeadline);
        ...
        private static List<Code> GetCodeSet(IEnumerable<CodeSet> codeSetCollection, string codeSetId)
        {
            var mySet = codeSetCollection.FirstOrDefault(codeSet => codeSet.Id.Equals(codeSetId, StringComparison.InvariantCultureIgnoreCase));
            if (mySet == null)
            {
                return null;
            }
            return mySet.CodeCollection.Select(code => new Code {id = code.Id, value = code.Value}).ToList();
        }
    }
}