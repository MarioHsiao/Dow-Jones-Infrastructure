		...
		*/
		...
		show: function () {
			...
			if (this.options.maxHeight != 'auto') {
				this.$menu.css('maxHeight', this.options.maxHeight);
				this.$menu.css('overflow-y', 'auto');
				this.$menu.css('overflow-x', 'hidden');
			}

			var $menuScrollHeight = this.$menu.get(0).scrollHeight;
			var $menuHeight = this.$menu.height();

			if ($menuScrollHeight > $menuHeight && ($menuScrollHeight - $menuHeight > 16)) {
				this.$menu.width(this.$menu.width() + 1 + 'px');
				if ($('html').is('.ie7, .ie8, .ie9')) {
					this.$menu.css('overflow-y', 'scroll');
					this.$menu.width(this.$menu.width() + 'px');
				}
			}

			//IE hack to make the items width the full length of the menu
			if ($('html').is('.ie7, .ie8, .ie9')) {
				this.$menu.children('.menuitems').width(this.$menu.width());
			}
