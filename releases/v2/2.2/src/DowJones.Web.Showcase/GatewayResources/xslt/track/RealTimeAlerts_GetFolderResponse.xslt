  ...
  <msxsl:script language="JScript" implements-prefix="user">
    <![CDATA[		
		function RemoveXSI(strOrig)
		{
			...
			// NN - Old code - .replace is invalid anyway so the line below does not execute
			// RetVal = temp.replace ("xsi:nil=\"true\"", "");			
			// NN - New code - removes all nodes with xsi:nil="true"
			RetVal = temp.replace(/<\w+\s+xsi:nil=\"true\"\s*\/>/g,"");
			return RetVal;			
		}
		]]>
  </msxsl:script>
