...
(function ($) {
	...
	DJ.UI.AbstractCanvas = DJ.UI.CompositeComponent.extend({
		...
		_sortableItems: {},
		_pubSubManager: this._pubSubManager || new DJ.Platform.PubSubManager(),
		...
		loadModule: function (moduleId) {
			...
			if ($dj.globalHeaders && $dj.globalHeaders.credentials)
				request['SA_FROM'] = $dj.globalHeaders.credentials.SA_FROM;
		...
		deleteModule: function (moduleId, onSuccess, onError) {
			...
			var success =
                $dj.delegate(this,
                    function (obj) {
                    	this._initializeModules();
                    	// initializing moves the modules implictly. fire a move
                    	this._fireModuleMoved();
		...
		_fireModuleMoved: function () {
			var modules = this.getModules();
			_.each(modules, function (module) {
				if (module === null) { return; }
				module.fireOnModuleMove();
			});
		},
		...
		_fireSortableOnDeactivate: function (e, ui) {
			...
			var module = ui.item.findComponent(DJ.UI.AbstractCanvasModule);
			if (module) {
				module.fireOnModuleMove();
			}
		},
