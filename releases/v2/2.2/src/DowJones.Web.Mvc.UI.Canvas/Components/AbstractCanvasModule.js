...
DJ.UI.AbstractCanvasModule = DJ.UI.CompositeComponent.extend({
	...
	init: function (element, meta) {
		...
		this._registerOwnerOnChildComponents();
	},
	...
	_registerOwnerOnChildComponents: function () {
		if (!this.childComponents) { $dj.info('_registerOwnerOnChildComponents: No child components found'); return; }

		for (var i = 0, len = this.childComponents.length; i < len; i++) {
			this.$element.find('#' + this.childComponents[i].id).findComponent(DJ.UI.Component).setOwner(this);
		}
	},

	// cleanup components and any other resources.
	// modules should override this to cleanup specific resources
	dispose: function () {
		this._super();
