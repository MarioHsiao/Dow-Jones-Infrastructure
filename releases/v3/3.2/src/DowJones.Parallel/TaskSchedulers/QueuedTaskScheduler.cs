    ...
    public sealed class QueuedTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        private class QueuedTaskSchedulerDebugView
        {
            ...
            public IEnumerable<Task> ScheduledTasks
            {
                get
                {
                    var tasks = (scheduler.targetScheduler != null) ?
                        scheduler.nonthreadsafeTaskQueue :
                        (IEnumerable<Task>)scheduler.blockingTaskQueue;
        ...
        /// are preferred.  Priority groups are round-robin through in order of priority.
        /// </summary>
        ...
        private readonly Queue<Task> nonthreadsafeTaskQueue;
        ...
        private readonly BlockingCollection<Task> blockingTaskQueue;
        ...
        /// <summary>Initializes the scheduler.</summary>
        /// <param name="targetScheduler">The target underlying scheduler onto which this scheduler's work is queued.</param>
            ...
            int maxConcurrencyLevel)
        {
            ...
            if (maxConcurrencyLevel < 0) throw new ArgumentOutOfRangeException("maxConcurrencyLevel");
        ...
        private int DebugTaskCount
        {
            get
            {
                return (targetScheduler != null ? 
                    nonthreadsafeTaskQueue : (IEnumerable<Task>)blockingTaskQueue)
        ...
        protected override void QueueTask(Task task)
        {
            ...
            else
            {
                ...
                lock (nonthreadsafeTaskQueue)
                {
                    ...
                    if (delegatesQueuedOrRunning < _concurrencyLevel)
                    {
            ...
            while (!disposeCancellation.IsCancellationRequested && continueProcessing)
            {
                try
                {
                    ...
                    while (!disposeCancellation.IsCancellationRequested)
                    {
                        ...
                        lock (nonthreadsafeTaskQueue)
                        {
                            if (nonthreadsafeTaskQueue.Count == 0) return;
        ...
        protected override IEnumerable<Task> GetScheduledTasks()
        {
            ...
            return targetScheduler == null ? blockingTaskQueue.Where(t => t != null).ToList() : nonthreadsafeTaskQueue.Where(t => t != null).ToList();
        }
        ...
        public TaskScheduler ActivateNewQueue(int priority)
        {
            ...
            {
                QueueGroup list;
                if (!queueGroups.TryGetValue(priority, out list))
                {
                    list = new QueueGroup();
        ...
        private sealed class QueuedTaskSchedulerQueue : TaskScheduler, IDisposable
        {
            ...
            private sealed class QueuedTaskSchedulerQueueDebugView
            {
                ...
                public int Priority { get { return _queue.Priority; } }
                ...
                public QueuedTaskScheduler AssociatedScheduler { get { return _queue.pool; } }
            }
            ...
            internal int WaitingTasks { get { return WorkItems.Count; } }
            ...
            /// <summary>Gets the maximum concurrency level to use when processing tasks.</summary>
            public override int MaximumConcurrencyLevel { get { return pool.MaximumConcurrencyLevel; } }
