/*
This library contains alteractions by the UX team.
	
Update to legend rendering, always use simpleSymbol method.
lines affected: 7067

Update to legend path rendering top offset, set negative y values for IE.
lines affected: 7097, 7100
	
Options have been added for 
legend -> symbolHeight : this compliments http://www.highcharts.com/ref/#legend--symbolWidth
lines affected: 1141, 6894, 7086, 7113
legend -> itemHeight : this compliments http://www.highcharts.com/ref/#legend--itemWidth
lines affected: 7172 
*/
// ==ClosureCompiler==
        ...
        */
        function renderItem(item) {
            ...
            if (!li) { // generate it once, later move it
                ...
                simpleSymbol = false; //UX updated, always use simpleSymbol /^(bar|pie|area|column)$/.test(series.type);
                ...
                if (!simpleSymbol && itemOptions && itemOptions.lineWidth) {
                    var attrs = {
                        'stroke-width': symbolHeight || itemOptions.lineWidth,
                        zIndex: 2
                    };
            ...
            itemHeight = options.itemHeight || bBox.height;
    ...
    render: function () {
        ...
        if (!series.group) {
            ...
            if (chart.inverted) {
                ...
                // dacostad: changed the following 
                // from : addEvent(chart, 'resize', setInvert); // do it on resize
                //        addEvent(series, 'destroy', function () {
                //          removeEvent(chart, 'resize', setInvert);
                //        });
                // to:
                try {
                    addEvent(win, 'resize', setInvert); // do it on resize
                }
                catch(ex) {
                }               
                addEvent(series, 'destroy', function () {
                    removeEvent(win, 'resize', setInvert);
                });
                // end change
            }
