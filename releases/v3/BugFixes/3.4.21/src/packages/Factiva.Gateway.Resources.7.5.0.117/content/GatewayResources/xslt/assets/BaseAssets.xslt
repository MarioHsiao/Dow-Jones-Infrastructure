...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:user="user" xmlns:fcp="urn:factiva:fcp:v2_0" xmlns:msxsl="urn:schemas-microsoft-com:xslt" extension-element-prefixes="msxsl user fcp xsl" xmlns="http://global.factiva.com/fvs/1.0">
      ...
      <xsl:for-each select="/GetArchiveObjectResponse/ResultSet/Result/DistDoc">
        ...
        <xsl:if test="./ArchiveDoc/overviewAndHistory">
          <assets xsi:type="OverviewAndHistoryAsset">
            <metaData>
              <xsl:choose>
                <xsl:when test="number(normalize-space(@distdocver))>=8">
                  <xsl:apply-templates select="./MetadataPT" mode="DD8"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:if test="./ArchiveDoc/cashFlow">
          <assets xsi:type="CashFlowAsset">
            ...
            <assetData xsi:type="CashFlowAssetData">
              <cashFlow>
                <xsl:copy-of select="./ArchiveDoc/cashFlow/@*"/>
                        ...
                        <xsl:for-each select="./*">
                          <xsl:choose>
                            <xsl:when test="local-name()='items'">
                              <xsl:element name="items">
                                <xsl:choose>
                                  <xsl:when test="./@xsi:type != 'MultexCashFlowItems'">
                                    <xsl:copy-of select="./@xsi:type"/>
                                  </xsl:when>
                                </xsl:choose>
                                <xsl:for-each select="./*">
  ...
  <!--MetadataPTDD8 template						-->
  <xsl:template name="metaDataPTDD8" match="MetadataPT" mode="DD8">
    <accessionNo fid="an" value="{./Properties/PropertySet[@group='docdata']/Property[@name='accessionno']/@value}"/>
    <baseLanguage>
      <xsl:value-of select="./Properties/PropertySet[@group='docdata']/Property[@name='baselang']/@value"/>
    </baseLanguage>
    <xsl:if test="string-length ( normalize-space( ./Properties/PropertySet[@group='docdata']/Property[@name='pubdate']/@value ) ) > 0">
      <publicationDate>
        <xsl:value-of select="./Properties/PropertySet[@group='docdata']/Property[@name='pubdate']/@value "/>
      </publicationDate>
    </xsl:if>
    <xsl:if test="string-length ( normalize-space( ./Properties/PropertySet[@group='docdata']/Property[@name='moddate']/@value ) ) > 0">
      <modificationDate>
        <xsl:value-of select="./Properties/PropertySet[@group='docdata']/Property[@name='moddate']/@value"/>
      </modificationDate>
    </xsl:if>
    <xsl:call-template name="ProviderObject">
      <xsl:with-param name="providerId">
        <xsl:value-of select="./Properties/PropertySet[@group='pubdata']/Property[@name='srccode']/@value"/>
      </xsl:with-param>
    </xsl:call-template>
    <xsl:for-each select="./Codes/CodeSet[@codeCat='co']/*">
      <company code="{./@value}">
        <xsl:if test="string-length( normalize-space( ./CodeD) ) > 0">
          <name>
            <xsl:value-of select="./CodeD"/>
          </name>
        </xsl:if>
      </company>
    </xsl:for-each>
    <xsl:for-each select="./Codes/CodeSet[@codeCat='in']/*">
      <industry code="{./@value}">
        <xsl:if test="string-length( normalize-space( ./CodeD) )  > 0">
          <name>
            <xsl:value-of select="./CodeD"/>
    ...
    <xsl:for-each select="./Codes/CodeSet[@codeCat='re']/*">
      <region code="{./@value}">
        <xsl:if test="string-length( normalize-space( ./CodeD ) ) > 0">
          <name>
            <xsl:value-of select="./CodeD"/>
  ...
  <xsl:template name="metaData" match="MetadataPT" mode="base">
    ...
    <xsl:for-each select="./CodeSets/CSet[@fid='re']/*">
      <region code="{./@value}">
        <xsl:if test="string-length( normalize-space( ./CodeD/@value ) ) > 0">
          <name>
            <xsl:value-of select="./CodeD/@value"/>
