    ...
    internal class RssFeedConverter : AbstractHeadlineListDataResultSetConverter
    {
        ...
        private HeadlineInfo Convert(RssItem item, int index, string language)
        {
            ...
            if (item != null)
            {
                ...
                headlineInfo.importanceDescriptor = ResourceText.GetAssignedToken(Importance.Normal);

                headlineInfo.reference.guid = (item.Guid != null) ? item.Guid.Value : new UrlBuilder(item.Link.ToString()).ToString();
                headlineInfo.reference.type = RssToken;
                headlineInfo.reference.externalUri = (item.Guid != null && item.Guid.IsPermanentLink) ? item.Guid.Value : new UrlBuilder(item.Link.ToString()).ToString(); 
            }
