...
namespace DowJones.Managers.Alert
{
    ...
    public class AlertManager : AbstractAggregationManager
    {
        ...
        private void Map(FolderDetails source, AlertProperties target)
        {
            ...
            if (source.deliveryMethod == DeliveryMethod.Continuous)
            {
                target.NewDeliveryTimes = new List<DeliveryTimes> { DeliveryTimes.Continuous };
            }
            else if (source.deliveryMethod == DeliveryMethod.Online)
            {
                target.NewDeliveryTimes = new List<DeliveryTimes> { DeliveryTimes.None };
            }
            else
            {
                target.NewDeliveryTimes = source.newDeliveryTimes;
            }
            ...
            target.PressClipsOnly = source.pressClipSrcBlock;
        }

        private static DeliveryMethod MapDeliveryMethod(List<DeliveryTimes> NewDeliveryTimes)
        {

            if (NewDeliveryTimes != null)
            {
                if (NewDeliveryTimes.Count == 1
                    && (NewDeliveryTimes[0] == DeliveryTimes.Continuous || NewDeliveryTimes[0] == DeliveryTimes.None))
                {
                    return (NewDeliveryTimes[0] == DeliveryTimes.Continuous) ? DeliveryMethod.Continuous : DeliveryMethod.Online;
                }
                if (NewDeliveryTimes.FindIndex(d => (d == DeliveryTimes.Afternoon || d == DeliveryTimes.Morning || d == DeliveryTimes.EarlyMorning)) != -1)
                {
        ...
        private void Map(AlertProperties source, TrackFolder target)
        {
            ...
            target.deliveryMethod = MapDeliveryMethod(source.NewDeliveryTimes);
            ...
            target.pressClipSrcBlock = source.PressClipsOnly;

            if (target.deliveryMethod == DeliveryMethod.Batch)
            {
                target.maxNumber = 200;//if the deliveryMethod is Batch set the maxNumber to 200
            }
