...
namespace DowJones.Managers.Search
{
    ...
    public partial class SearchManager
    {
        ...
        /// <typeparam name="TIPerformContentSearchResponse">The type of the I perform content search response.</typeparam>
            ...
            if (dto.IsValid())
            {
                ...
                if (!dto.HasDocumentVector)
                {
                        ...
                        accResponse.ContentSearchResult.ContentHeadlineResultSet.Count == 1)
                    {
                        ...
                        currentContentServerAddress = LastTransactionControlData != null ? LastTransactionControlData.ContentServerAddress : (ushort)0;
                    }
                    else
                    {
                ...
                var temp = ControlDataManager.Clone(ControlData);
                temp.ContentServerAddress = currentContentServerAddress;

                return GetPerformContentSearchResponse<TIPerformContentSearchRequest,TIPerformContentSearchResponse>(temp, dto);
            }
