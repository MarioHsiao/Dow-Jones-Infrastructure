...
namespace System.Threading.Tasks.Schedulers
{
    ...
    public sealed class RoundRobinSchedulerGroup
    {
        ...
        {
            internal RoundRobinTaskSchedulerQueue(RoundRobinSchedulerGroup pool) { this.pool = pool; }
            ~RoundRobinTaskSchedulerQueue()
            {
                Dispose(false);
            }

            private readonly RoundRobinSchedulerGroup pool;
            internal readonly Queue<Task> WorkItems = new Queue<Task>();
            internal bool Disposed;

            protected override IEnumerable<Task> GetScheduledTasks() 
            { 
                object obj = pool._queues;
            ...
            protected override void QueueTask(Task task)
            {
                ...
                lock (pool._queues) WorkItems.Enqueue(task);
            ...
            private void Dispose(bool disposing)
            {
                ...
                lock (pool._queues)
                {
                    if (WorkItems.Count == 0) pool.RemoveQueue_NeedsLock(this);
                    Disposed = true;
                }
            }
