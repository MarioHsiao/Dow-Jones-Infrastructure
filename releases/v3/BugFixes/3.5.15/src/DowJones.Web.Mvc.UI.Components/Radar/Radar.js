...
DJ.UI.Radar = DJ.UI.Component.extend({
    ...
    */
    ...
    _getNewsEntityReference: function (data) {
        var ref = data.parentNewsEntities[0].newsEntities;
        for (var i = 1; i < data.parentNewsEntities.length; i++) {
            if (ref.length < data.parentNewsEntities[i].newsEntities.length) {
                ref = data.parentNewsEntities[i].newsEntities;
            }
        }
        return ref;
    },
