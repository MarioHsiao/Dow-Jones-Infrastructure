...
namespace System.Threading.Async
{
    ...
    public sealed class AsyncSemaphore : IDisposable
    {
        ...
        public AsyncSemaphore(int initialCount, int maxCount)
        {
            ...
            _maxCount = maxCount;
            waitingTasks = new Queue<TaskCompletionSource<object>>();
        }
        ...
        public int WaitingCount { get { lock(waitingTasks) return waitingTasks.Count; } }
        ...
        public void Dispose()
        {
            if (_maxCount > 0)
            {
                ...
                lock (waitingTasks)
                {
                    ...
                    {
                        waitingTasks.Dequeue().SetCanceled();
                    }
                }
            }
        }
