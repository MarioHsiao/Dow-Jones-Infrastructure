...
using DowJones.Utilities.Managers.Search.Comparers;
        ...
        public AccessionNumberSearchResponse PerformAccessionNumberSearch<TIPerformSearchRequest, TIPerformSearchResponse>(AccessionNumberSearchRequestDTO dto) where TIPerformSearchRequest : IPerformContentSearchRequest, new() where TIPerformSearchResponse : IPerformContentSearchResponse, new()
        {
            if (dto.IsValid())
            {
                ...
                if (response != null)
                {
                    ...
                    if (data.Count == uniqueAccessionNumbers.Length)
                    {
                        ...
                        output.CanReturnDiscoveryData = true;
                    ...
                    // run search against the historical Database
                    var originalAccessionNo = dto.AccessionNumbers;
                    var missingAccessionNo = new List<string>();
                    foreach (var s in uniqueAccessionNumbers)
                    {
                        if (!data.ContainsKey(s))
                        {
                            missingAccessionNo.Add(s);
                        }
                    }
                    
                    // update the dto w/missing accessionNos
                    dto.AccessionNumbers = missingAccessionNo.ToArray();
                    
                    // instruct the dto to use all dates [Note: this is an internal property]
                    dto.UseAllDates = true;

                    // fire the search
                    response = GetPerformContentSearchResponse<TIPerformSearchRequest, TIPerformSearchResponse>(dto);
                    if (response != null)
                    {
                        data = SearchUtility.GetDictionaryOfContentHeadlines(data, response);
                    }

                    // backfill the object with the original accession Nos.
                    dto.AccessionNumbers = originalAccessionNo;

                    if (data != null)
                    {
                        var items = GetAccessionNumberBasedContentItems(data, uniqueAccessionNumbers, dto.SortBy);

                        // tell user that discovery is not available 
                        output.CanReturnDiscoveryData = false;
                        output.AccessionNumberBasedContentItemSet.AccessionNumberBasedContentItemCollection.AddRange(items);
                ...
                // send an object that basically has no data in it.  Should never hit this condition.
                output.CanReturnDiscoveryData = false;
