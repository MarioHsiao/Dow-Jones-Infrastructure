...
using DowJones.Utilities.Formatters.Globalization.Cultures;
...
namespace DowJones.Utilities.Formatters.Globalization
{
    ...
    public class DateTimeFormatter : IFormatter<Date>
    {
        ...
        /// </returns>
        public string FormatStandardDate(DateTime dt)
        {
            return FormatStandardDate(dt, false);
        }
        ...
        /// The format standard date.
        /// </returns>
        public string FormatStandardDate(DateTime dt, bool usePreference)
        {
            DateTime temp = ConvertToUtc(dt);
        ...
        public string FormatTime(string s)
        {
            ...
            if (DateTime.TryParse(s, CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.AdjustToUniversal | DateTimeStyles.NoCurrentDateDefault | DateTimeStyles.AssumeUniversal, out dateTime))
        ...
        /// </returns>
        private string CoreFormatTime(DateTime dt)
        {
            DateTime temp = ConvertToUtc(dt);
        ...
        /// <returns>
        /// </returns>
        private static DateTime ConvertToUtc(DateTime date, DateTime time)
        {
            ...
            var baseDateTime = new DateTime(1, 1, 3, time.Hour, time.Minute, time.Second, time.Millisecond, DateTimeKind.Local);

            // Check to see if the system is set to a time-zone that is using Daylight
            if (DateTime.Now.IsDaylightSavingTime())
            {
                // Subtract the delta.
                DaylightTime daylightTimes = System.TimeZone.CurrentTimeZone.GetDaylightChanges(baseDateTime.Year);
                baseDateTime = baseDateTime.Subtract(daylightTimes.Delta);
            }

            // Generate a good UTC time 
            DateTime temp = baseDateTime.ToUniversalTime();
            temp = new DateTime(temp.Ticks, DateTimeKind.Utc);

            // output the correct combined UTC
            var nDate = new DateTime(date.Year, date.Month, date.Day, temp.Hour, temp.Minute, temp.Second, temp.Millisecond, DateTimeKind.Utc);
            return nDate;
        }
        ...
        {
            // This is done to guarantee that this is a universal date time.
            return DateTime.TryParse(s, CultureInfo.InvariantCulture.DateTimeFormat, DateTimeStyles.AdjustToUniversal | DateTimeStyles.NoCurrentDateDefault | DateTimeStyles.AssumeUniversal, out date);
        }
        ...
        {
            return date.Kind == DateTimeKind.Utc ? date : new DateTime(date.Year, date.Month, date.Day, date.Hour, date.Minute, date.Second, date.Millisecond, DateTimeKind.Utc);
        }
        ...
        /// The parse time.
        /// </returns>
        public static bool ParseTime(string s, out DateTime time)
        {
            if (string.IsNullOrEmpty(s) || string.IsNullOrEmpty(s.Trim()))
            {
        ...
        {
            bool isSuccessfulDateParsing = false;

            bool isSuccessfulTimeParsing = false;
            ...
            {
                return new DateTime(datePortion.Year, datePortion.Month, datePortion.Day, timePortion.Hour, timePortion.Minute, timePortion.Second, DateTimeKind.Utc);
            }

            if (isSuccessfulDateParsing)
            {
                return new DateTime(datePortion.Year, datePortion.Month, datePortion.Day, datePortion.Hour, datePortion.Minute, datePortion.Second, DateTimeKind.Utc);
            }
        ...
        /// </returns>
        private string CoreFormatShortDateTime(DateTime dt)
        {
            DateTime temp = ConvertToUtc(dt);
            if (timeZoneBuilder.ConvertToLocalTime)
            {
        ...
        {
            DateTime dt;
            return ParseDate(s, out dt) ? FormatDateTime(dt) : CoreFormatShortDateTime(DateTime.MinValue);
        }
        ...
        /// </returns>
        private string CoreFormatLongDateTime(DateTime dt)
        {
            DateTime temp = ConvertToUtc(dt);
            if (timeZoneBuilder.ConvertToLocalTime)
            {
        ...
        {
            DateTime dt;
            return ParseDate(s, out dt) ? FormatDateTime(dt) : CoreFormatLongDateTime(DateTime.MinValue);
        }
        ...
        {
            DateTime dt;
            return ParseDate(s, out dt) ? FormatDateTime(dt) : CoreStandardHeadlineFormatDateTime(DateTime.MinValue);
        }
        ...
        /// The format standard headline date time.
        /// </returns>
        public string FormatStandardHeadlineDateTime(DateTime date, DateTime time)
        {
            DateTime dt = Merge(date, time);
            return FormatStandardHeadlineDateTime(dt);
        }
        ...
        /// The format standard headline date time.
        /// </returns>
        public string FormatStandardHeadlineDateTime(DateTime dateTime)
        {
        ...
        {
            DateTime dt;
            return ParseDate(s, out dt) ? FormatDateTime(dt) : CoreFormatDateTime(DateTime.MinValue);
        }
        ...
        /// The format standard date time.
        /// </returns>
        public string FormatStandardDateTime(DateTime dateTime)
        {
        ...
        /// </returns>
        private string CoreFormatDateTime(DateTime dt)
        {
            DateTime temp = ConvertToUtc(dt);
            if (timeZoneBuilder.ConvertToLocalTime)
            {
        ...
        /// </returns>
        private string CoreStandardHeadlineFormatDateTime(DateTime dt)
        {
            DateTime temp = ConvertToUtc(dt);
            if (timeZoneBuilder.ConvertToLocalTime)
            {
