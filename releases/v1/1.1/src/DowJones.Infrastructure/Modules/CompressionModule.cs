...
namespace DowJones.Utilities.Modules
{
    public class CompressionModule : IHttpModule
    {
        ...
        private static bool CanCompressScriptResource(HttpRequest request)
        {
            if (Settings.Default.EnableScriptResourceCompression && request.Url.Segments.Length > 0)
            {
                if (request.Url.Segments[request.Url.Segments.Length - 1].ToLowerInvariant() == "scriptresource.axd")
                {
        ...
        private void PreRequestHandlerExecute(object sender, EventArgs e)
        {
            ...
            else if (CanCompressScriptResource(request))
            {
