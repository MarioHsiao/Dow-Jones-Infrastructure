...
namespace DowJones.Utilities.DTO.Web.JSON
{
	...
	public class JSONObject
	{
		...
		#region Getters for a value associated with a key - use indexer instead
		...
		{
			return (string[]) myKeyIndexList.ToArray(typeof (string));
		}
		...
		public string numberToString(object number)
		{
            ...
            if (number is float)
            {
               s = ((float)number).ToString(CultureInfo.InvariantCulture.NumberFormat).ToLower();
            }
            if (number is double)
            {
               s = ((double)number).ToString(CultureInfo.InvariantCulture.NumberFormat).ToLower();
            }
		...
		public override string ToString()
		{
			...
			foreach (string key in myHashMap.Keys) //NOTE! Could also use myKeyIndexList !!!
			{
				...
				if (obj != null)
				{
					// ORIGINAL IMPLEMENTATION PERFORMS ENCODING ON KEYS FOR THE FOLLOWING CHARACTERS
					//	{ } [ ] / \ : , 
					// WE WILL ASSUME "NICE" KEYS AND ASSUME THAT WE WILL ALWAYS GET "NICE" KEYS
		...
		/// <summary>
		/// Returns a Base64 encoded string format of the current object
		/// </summary>
		...
		public string ToBase64String()
		{
			...
			byte[] bytes = enc.GetBytes(str.ToCharArray());
