...
namespace DowJones.Web.Mvc.UI
{
    ...
    public class ClientStateMapper : IClientStateMapper
    {
        public ClientState Map(object source)
        {
            ...
            var clientDataAttribute = clientStateAttributes.SingleOrDefault(state => state.Attribute is ClientDataAttribute);
            if (clientDataAttribute != null)
                clientState.Data = clientDataAttribute.Property.GetValue(source, null);
        ...
        private static object GetPropertyValue(ClientStateInfo clientStateInfo, object source)
        {
            ...
            {
                Type jsonTypeConverterType = BuildManager.GetType(jsonConverterAttribute.ConverterType.FullName, true);
                var typeConverter = ServiceLocator.Resolve(jsonTypeConverterType) as TypeConverter;
