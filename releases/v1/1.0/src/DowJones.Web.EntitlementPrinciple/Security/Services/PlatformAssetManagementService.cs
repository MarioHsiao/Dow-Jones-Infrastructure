...
namespace DowJones.Web.EntitlementPrinciple.Security.Services
{
    ...
    public class PlatformAssetManagementService : AbstractService, IPlatformAssetManagementService
    {
        ...
        internal override sealed void Initialize()
        {
            ...
            //AC's
            NumberOfPersonalNewspages = ListUtility.MatchPatternForNewsPage(_matrixPAMService.ac1, @"newspage:([0-9\-]+)", 1);
            if(!(NumberOfPersonalNewspages < 0) && NumberOfPersonalNewspages < 1)
            {
                IsNewspagesSubscribeOnly = ListUtility.FindInList(_matrixPAMService.ac1, "newspage:0");
            }
