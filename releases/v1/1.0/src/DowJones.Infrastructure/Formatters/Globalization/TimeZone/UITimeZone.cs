...
namespace DowJones.Utilities.Formatters.Globalization.TimeZone
{
    ...
    public class UITimeZone : System.TimeZone, IComparable<System.TimeZone>, IComparable, IDeserializationCallback
    {
        ...
        /// <returns></returns>
        private TimeSpan GetUtcOffset(DateTime time, bool adjustToDaylightSavingsTime)
        {
        ...
        protected TimeSpan GetUtcOffsetFromUniversalTime(DateTime time)
        {
            var year = time.Year;
            var daylightTimes = GetDaylightChanges(year);
            var settings = GetBiasSettings(year);
        ...
        /// <param name="adjustToDaylightSavingsTime">if set to <c>true</c> [adjust to daylight savings time].</param>
        /// <returns></returns>
        private TimeSpan GetUtcOffsetFromUniversalTime(DateTime time, bool adjustToDaylightSavingsTime)
        {
            var compareTime = DateTime.MinValue;
            switch (time.Kind)
            {
                ...
                case DateTimeKind.Utc:
                    compareTime = new DateTime(time.Year, time.Month, time.Day, time.Hour, time.Minute, time.Second, time.Millisecond, DateTimeKind.Local);
                    break;
            }
        ...
        /// <param name="adjustToDaylightSavingsTime">if set to <c>true</c> [adjust to daylight savings time].</param>
        /// <returns></returns>
        public DateTime ToLocalTime(DateTime time, bool adjustToDaylightSavingsTime)
        {
            ...
            if (ticks > max)
            {
                return new DateTime(max, DateTimeKind.Local);
            }

            return ticks < 0 ? new DateTime(0, DateTimeKind.Local) : new DateTime(ticks, DateTimeKind.Local);
        }
        ...
        private class BiasSettings
        {
            ...
            /// </summary>
            private static DateTime GetParticularDay(int year, DaylightLimit rule)
            {
                if (!rule.Enabled) return DateTime.MinValue;

                // Get the first occurence of the requested weekday
                var result = new DateTime(year, rule.Month, 1,
