...
namespace DowJones.Tools.Ajax.Converters.HeadlineList
{
    public abstract class AbstractHeadlineListDataResultSetConverter : IListDataResultConverter
    {
        ...
        protected string GetAssignedToken<T>(T value)
        {
            ...
            var assignedToken = (AssignedToken)Attribute.GetCustomAttribute(enumType.GetField(s), typeof(AssignedToken));
            return assignedToken != null ? resourceText.GetString(assignedToken.Token) : string.Empty;
        }
        ...
        protected internal string GetLanguageToContentLanguage(string language)
        {
            if (!string.IsNullOrEmpty(language) && !string.IsNullOrEmpty(language.Trim()))
            {
                ...
                if (fieldInfo != null)
                {
                    ...
                    {
                        return resourceText.GetString(assignedToken.Token);
                    }
                }
            }
            return language;
                ...
                case ContentCategory.Picture:
                    switch (info.contentSubCategory)
                    {
                        case ContentSubCategory.Graphic:
                            foreach (var item in headline.ContentItems.ItemCollection)
                            {
                                if (string.IsNullOrEmpty(item.Type) ||
                                    string.IsNullOrEmpty(item.Type.Trim()) ||
                                    item.Type.ToLower() != "tnail")
                                {
                                    continue;
                                }

                                info.reference.@ref = item.Ref;
                                info.reference.mimetype = (item.Mimetype != null) ? item.Mimetype.ToLower() : string.Empty;
                            }
