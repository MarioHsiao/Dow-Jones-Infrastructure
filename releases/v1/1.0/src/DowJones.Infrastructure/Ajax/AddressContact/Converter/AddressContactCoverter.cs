...
using DowJones.Utilities.Ajax.AddressContact;

namespace DowJones.Utilities.Ajax.Converter.AddressContact
{
        ...
        public AddressContactData Process()
        {
            ...
            if (companyArchive != null)
            {
                ...
                foreach (Factiva.Gateway.Messages.FCE.Assets.V1_0.WebSite ws in webSiteCollection)
                {
                    ...
                    {
                        case "HPG":
                            data.CompanyWebSite = new DowJones.Utilities.Ajax.AddressContact.WebSite()
                            {
                                Url = ws.Url,
                        ...
                        case "PAS":
                        case "EMP":
                            data.AdditionalCompanyLinks.Add(new DowJones.Utilities.Ajax.AddressContact.WebSite()
                            {
                                Url = ws.Url,
                                Description = ws.Category.Descriptor.Value,
