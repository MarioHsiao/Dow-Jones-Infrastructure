    ...
    public class AbstractStructuredQuery : IStructuredQuery
    {
        ...
        protected NameListResult GetNicknames(string firstName)
        {
            ...
            if (firstName != null)
            {
                ...
                var serviceResponse = SymbologyNameListService.PerformNameListSearch(m_ControlData, performNameListSearchRequest);
        ...
        protected CompanyFilter GetCompaniesFromScreening(string[] fcodes, ControlData controlData)
        {
            //********Replaced old gateway calls with current gateway calls.********//
            ...
            getReportListExRequest.FirstResultToReturn = 0;
            getReportListExRequest.MaxResultsToReturn = fcodes.Length;
            getReportListExRequest.ReturnExistenceFlags = true;
            getReportListExRequest.ReturnReplyItem = false;
            getReportListExRequest.ReturnRestrictors = false;
            getReportListExRequest.SortBy = ReportListSortBy.PublishedDate;
            getReportListExRequest.SortOrder = CompanyProfilesSortOrder.Ascending;
            getReportListExRequest.Subtype.StringCollection.Add(string.Empty);
            getReportListExRequest.Category = ReportCategory.Company;
            getReportListExRequest.Fcode = fcodes[0];
            getReportListExRequest.LanguageList = Language;
            getReportListExRequest.SymbolCodeScheme = SymbolCodeScheme.FII;

            var serviceResponse = ScreeningService.GetReportListEx(controlData, getReportListExRequest);
        ...
        protected CompanyFilter GetCompanies(string[] fcodes)
        {
            ...
            var serviceResponse = SymbologyCompanyService.GetCompanies(ControlData, getCompaniesRequest);
            if (serviceResponse.rc != 0)
                throw new DowJonesUtilitiesException(serviceResponse.rc);
