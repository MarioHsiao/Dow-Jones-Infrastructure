...
namespace DowJones.Web.Mvc.UI.Canvas.DataAccess.ServiceResult.Multimedia
{
    ...
    public class MultimediaServiceResult : AbstractServiceResult, IPopulate<VideoRequest>
    {
        ...
        private const string DEFAULT_RAMP_REDIRECT = "http://factiva.ramp.com/redirect/?e=";
        ...
        private void GetData(ControlData controlData,VideoRequest request)
        {
            ...
            MustPlayFromSource mustPlay = new MustPlayFromSource { Status = false, Url = DEFAULT_RAMP_REDIRECT + episodeId };

            string partOfId = request.AccessionNo.Substring(0, 8);
            if (partOfId.EndsWith("0"))
                sourceCode = partOfId.TrimEnd(new char[] { '0' }).ToUpper();

            List<string> sourceList = GetOptInSourceList(controlData);
        ...
        private List<string> GetOptInSourceList(ControlData controlData)
        {
            List<string> lstSources = new List<string>();
            string sourceCode = string.Empty;
            bool found = false;

            var performSourceSearchResponse = new PerformSourceSearchResponse();
            var performSourceSearchRequest = new PerformSourceSearchRequest();
            performSourceSearchRequest.searchString = "rst=toptin and ila=en"; //TODO move to a constant
            performSourceSearchRequest.resultType = ResultType.Sources;
            performSourceSearchRequest.responseType = ResponseType.Brief;

            ServiceResponse serviceResponse = Factiva.Gateway.Services.V1_0.SourcesService.PerformSourceSearch(ControlDataManager.Clone(controlData), performSourceSearchRequest);
            ...
            if (serviceResponse.rc == 0)
            {
                ...
                if (responseObjRC == 0)
                {
                    performSourceSearchResponse = (PerformSourceSearchResponse)responseObj;
